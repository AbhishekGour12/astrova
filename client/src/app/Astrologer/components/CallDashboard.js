...
enabled: isCallConnected && Boolean(zegoRoomId && astrologerId)
...
// A. Get Active Call (The one currently connected)

const activeRes = await api.get(`/call/astrologer/active/${id}`);

if (activeRes.data.success && activeRes.data.call) {

const call = activeRes.data.call;

setActiveCall(call);


// 🔥 LOGIC: Check status immediately

if (call.status === "ACTIVE") {

setIsCallConnected(true);

setZegoRoomId(call.zegoRoomId);
...
// B. Get Waiting Calls (Incoming requests)

const waitingRes = await api.get(`/call/astrologer/waiting/${id}`);

if (waitingRes.data.success) {

// 🔥 FIX: Ensure we map the data correctly so 'username' is available

const formattedCalls = waitingRes.data.calls.map(call => ({

...call,

username: call.user?.name || call.user?.username || "Unknown User", // Handle naming variations

profileImage: call.user?.profileImageUrl
...
}, [astrologerId, router, fetchPendingAndActiveCalls, refreshCalls]);
...
useEffect(() => {
  const handleVisibilityChange = () => {
    if (!document.hidden && isCallConnected && zegoRoomId) {
      console.log("🔁 Re-attaching Zego UI");

      // HARD RESET UI ONLY (call nahi)
...
// At the top of CallDashboard component

useEffect(() => {

// Make refreshCalls available globally for other components
...
<div className="min-h-screen bg-gradient-to-b from-gray-50 to-genabled: Boolean(zegoRoomId && astrologerId)
ray-100 flex items-center justify-center p-4">
